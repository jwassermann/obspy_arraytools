...
from obspy_arraytools.seismic_array import _get_stream_offsets
...
class SeismicArrayTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        def create_simple_array(coords, sys="xy"):
            ...
            if sys == "xy":
                coords_lonlat = [
                    util_lon_lat(0, 0, stn[0], stn[1]) for stn in coords
                ]
            else:
                coords_lonlat = coords
            stns = [
                Station(
                    str(_i), coords_lonlat[_i][1], coords_lonlat[_i][0], 0
                )  # flat!
                for _i in range(len(coords_lonlat))
            ]
            testinv = Inventory(
                [Network("testnetwork", stations=stns)], "testsender"
            )
    ...
    def test_get_geometry_xyz(self):
        ...
        geoxno3d = self.geometry_array._get_geometry_xyz(
            1, 1, 0, correct_3dplane=False
        )
        geox = self.geometry_array._get_geometry_xyz(
            1, 1, 0, correct_3dplane=True
        )
        # For flat array:
        ...
        for key_outer in geox_exp:
            for key_inner in geox_exp[key_outer]:
                self.assertAlmostEqual(
                    geox[key_outer][key_inner], geox_exp[key_outer][key_inner]
                )

    def test_center_of_gravity(self):
        self.assertEqual(
            self.geometry_array.center_of_gravity,
            {"absolute_height_in_km": 0.0, "latitude": 1.0, "longitude": 1.0},
        )

    def test_geometrical_center(self):
        self.assertEqual(
            self.geometry_array.geometrical_center,
            {"absolute_height_in_km": 0.0, "latitude": 1.0, "longitude": 1.0},
        )
    ...
    def test_inventory_cull(self):
        ...
        array = SeismicArray(
            "testarray",
            Inventory(
                [Network("BP", stations=[ccrb, eadb, wrong_stn])], "testinv"
            ),
        )
        ...
        tbc = [
            array.inventory.networks,
            array.inventory[0].stations,
            array.inventory[0][0].channels,
            array.inventory[0][1].channels,
        ]
    ...
    def test_covariance_array_processing(self):
        ...
        dt = np.round(
            ...
            * slowness
            * (
                np.cos(baz) * self.fk_testcoords[:, 1]
        ...
        out = self.fk_array._covariance_array_processing(
            *fk_args, prewhiten=0, method=0
        )
        raw = """
        ...
        self.assertTrue(
            np.allclose(ref, np.array(out[:, 1:], dtype=float), rtol=1e-6)
        )

        out = self.fk_array._covariance_array_processing(
            *fk_args, prewhiten=1, method=0
        )
        raw = """
    ...
    def test_fk_array_transff_freqslowness(self):
        transff = self.transff_array.array_transfer_function_freqslowness(
            40, 20, 1, 10, 1
        )
        transffth = np.array(
            [
    ...
    def test_three_component_beamforming(self):
        ...
        np.testing.assert_array_almost_equal(
            out.max_rel_power, 1.22923997, decimal=8
        )
    ...
    def test_plot_radial_transfer_function(self):
        ...
        fig = arr.plot_radial_transfer_function(
            0, 0.6, 0.05, [0.2], show=False
        )
        return fig
