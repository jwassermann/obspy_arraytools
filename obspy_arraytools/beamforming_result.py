...
def _plot_array_analysis(
    ...
    if baz_plot:
        ...
        xi = np.arange(0.0, maxslowg, sls)
        yi = np.arange(-180.0, 180.0, bzs)
        ...
        if baz_plot:
            ...
            for ix, sx in enumerate(slx):
                for iy, sy in enumerate(sly):
                    bbaz = np.arctan2(sx, sy) * 180 / np.pi + 180.0
                    if bbaz > 180.0:
                        bbaz = -180.0 + (bbaz - 180.0)
                    slowgrid.append(
                        (np.sqrt(sx * sx + sy * sy), bbaz, power[ix, iy])
                    )
            ...
            grid = interpolate.griddata(
                (sl, bz), slowg, (grid_x, grid_y), method="nearest"
            )
            ...
            slow_x = np.cos((baz[i] + 180.0) * np.pi / 180.0) * slow[i]
            slow_y = np.sin((baz[i] + 180.0) * np.pi / 180.0) * slow[i]
...
class BeamformerResult(object):
    ...
    def __init__(
        ...
        if self.timestep is not None:
            ...
            try:
                self.endtime = UTCDateTime(
                    win_starttimes[-1] + self.timestep,
        ...
        if (
            max_rel_power is None
            and max_pow_baz is None
            and max_pow_slow is None
            and full_beamres is not None
        ):
    ...
    def __add__(self, other):
        ...
        if any(
            (self.__dict__[attr] != other.__dict__[attr]) for attr in attrs
        ):
            raise ValueError(
                "Frequency and slowness range parameters must be " "equal."
            )
        ...
        if self.full_beamres is not None and other.full_beamres is not None:
            full_beamres = np.append(
                self.full_beamres, other.full_beamres, axis=2
            )
            max_rel_power, max_pow_baz, max_pow_slow, max_abs_power = (
                None,
                None,
                None,
                None,
            ...
            if self.max_abs_power is not None:
                max_abs_power = np.append(
                    self.max_abs_power, other.max_abs_power
                )
            else:
                max_abs_power = None
    ...
    def _get_plotting_timestamps(self, extended=False):
        ...
        newtimes = np.array(
            [t.timestamp / (24 * 3600) + 719163 for t in plot_times]
        )
        return newtimes
    ...
    def plot_baz_hist(self, show=True):
        ...
        abins = np.arange(azimuth_bins + 1) * 360.0 / azimuth_bins
        ...
        for i, row in enumerate(hist):
            ax.bar(
                x=(i * dw) * np.ones(slowness_bins),
                height=dh * np.ones(slowness_bins),
                width=dw,
                bottom=dh * np.arange(slowness_bins),
        ...
        ax.set_ylim(sll, slm)
        ColorbarBase(
            cax, cmap=cmap, norm=Normalize(vmin=hist.min(), vmax=hist.max())
        )
        plt.suptitle(
            "{} beamforming: results from \n{} to {}".format(
                self.method, self.starttime, self.endtime
            )
        )
        if show is True:
            plt.show()
    ...
    def plot_bf_results_over_time(self, show=True):
        ...
        for i, (data, lab) in enumerate(zip(datas, labels)):
            ...
            ax.scatter(
                self._get_plotting_timestamps(),
                data,
                c=self.max_rel_power,
                alpha=0.6,
                edgecolors="none",
                cmap=cm.viridis,
            )
            ax.set_ylabel(lab)
            timemargin = 0.05 * (
                self._get_plotting_timestamps()[-1]
                - self._get_plotting_timestamps()[0]
            )
            ax.set_xlim(
                self._get_plotting_timestamps()[0] - timemargin,
        ...
        fig.autofmt_xdate()
        fig.subplots_adjust(
            left=0.15, top=0.9, right=0.95, bottom=0.2, hspace=0.1
        )
        if show is True:
            plt.show()
    ...
    def plot_power(self, plot_frequency=None, show=True):
        ...
        maxazipows = np.array(
            [
                [azipows.T[t].max() for azipows in freqavg]
        ...
        for i, (data, lab) in enumerate(zip(datas, labels)):
            ...
            pc = ax.pcolormesh(
                self._get_plotting_timestamps(extended=True),
                azis,
                data.T,
                cmap=cm.viridis,
                rasterized=True,
            ...
            ax.set_xlim(
                self._get_plotting_timestamps()[0] - timemargin,
                self._get_plotting_timestamps(extended=True)[-1] + timemargin,
            )
            ax.set_ylim(0, 360)
            ax.yaxis.set_major_locator(ymajorlocator)
    ...
    def plot_bf_plots(
        ...
        if average_freqs is True and plot_frequencies is not None:
            warnings.warn(
                "Ignoring plot_frequencies, only plotting an average"
                " of all frequencies."
            )
        if (
            hasattr(plot_frequencies, "__getitem__") is False
            and plot_frequencies is not None
        ):
        ...
        def _actual_plotting(bfres, title):
            ...
            ax.contour(
                theo_backazi, self.slowness_range, bfres.T, 100, cmap=cmap
            )
            ax.set_theta_zero_location("N")
            ax.set_theta_direction(-1)
        ...
        if average_windows and not average_freqs:
            if plot_frequencies is None:
                ...
                for plot_freq in plot_frequencies:
                    ...
                    _actual_plotting(
                        np.squeeze(beamresnz[:, :, ifreq]),
                        "{} beamforming result, averaged over all"
                        " time windows\n for frequency {} Hz.".format(
                            self.method, self.freqs[ifreq]
                        ),
                    )
        ...
        if average_freqs and not average_windows:
            for iwin in range(len(beamresnz[0, 0, :])):
                _actual_plotting(
                    np.squeeze(beamresnz[:, :, iwin]),
                    "{} beamforming result, averaged over all "
                    "frequencies,\n for window {} "
                    "(starting {})".format(
                        self.method, iwin, self.win_starttimes[iwin]
                    ),
                )
            ...
            if plot_frequencies is None:
                ...
                for plot_freq in plot_frequencies:
                    ...
                    for iwin in range(len(beamresnz[0, 0, :, 0])):
                        _actual_plotting(
                            beamresnz[:, :, iwin, ifreq],
                            "{} beamforming result, for frequency"
                            " {} Hz,\n, window {} (starting {}).".format(
                                self.method,
                                self.freqs[ifreq],
                                iwin,
    ...
    def plot_radial_transfer_function(self, plot_freqs=None):
        ...
        plot_array.plot_radial_transfer_function(
            min(absolute_slownesses),
            max(absolute_slownesses),
            self.slowness_range[1] - self.slowness_range[0],
            plot_freqs,
        )
    ...
    def plot_transfer_function_freqslowness(self):
        ...
        arr.plot_transfer_function_freqslowness(
            max(absolute_slownesses),
            self.slowness_range[1] - self.slowness_range[0],
            min(self.freqs),
            max(self.freqs),
            abs(self.freqs[1] - self.freqs[0]),
        )
