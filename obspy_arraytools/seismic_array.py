...
def _get_stream_offsets(stream, stime, etime):
    ...
    for i, tr in enumerate(stream):
        if tr.stats.starttime > stime:
            msg = (
                "Specified stime %s is smaller than starttime %s " "in stream"
            )
        ...
        spoint[i] = int(
            (stime - tr.stats.starttime) * tr.stats.sampling_rate + 0.5
        )
        epoint[i] = int(
            (tr.stats.endtime - etime) * tr.stats.sampling_rate + 0.5
        )
    ...
    * Z positive up
    ...
    def inventory_cull(self, st):
        ...
        for k, netw in reversed(list(enumerate(inv.networks))):
            for j, stn in reversed(list(enumerate(netw.stations))):
                for i, cha in reversed(list(enumerate(stn.channels))):
                    if (
                        "{}.{}.{}.{}".format(
                            netw.code, stn.code, cha.location_code, cha.code
                        )
        ...
        if len(contents["channels"]) < len(stations_present):
            ...
            warnings.warn(
                "Inventory does not contain information for all "
                "traces in stream."
            )
        self.inventory = inv
    ...
    def plot(self, projection="local", show=True, **kwargs):
        ...
        fig = self.inventory.plot(
            projection=projection, show=False, method="basemap", **kwargs
        )
        bmap = fig.bmap
        ...
        bmap.scatter(
            x, y, marker="x", c="blue", s=100, zorder=201, linewidths=2
        )
        bmap.ax.text(
            x,
            y,
        ...
        bmap.scatter(
            x, y, marker="x", c="green", s=100, zorder=201, linewidths=2
        )
        bmap.ax.text(
            x,
            y,
        ...
        for network in self.inventory:
            for station in network:
                if len(station.channels) == 0:
                    ...
                    item_code = "{n}.{s}..".format(
                        n=network.code, s=station.code
                    )
                    this_coordinates = {
                        "latitude": float(station.latitude),
                        "longitude": float(station.longitude),
                        "absolute_height_in_km": float(station.elevation)
                        / 1000.0,
                    }
    ...
    def geometrical_center(self):
        ...
        extent = self.extent
        return {
            "latitude": (extent["max_latitude"] + extent["min_latitude"])
            / 2.0,
            "longitude": (extent["max_longitude"] + extent["min_longitude"])
            / 2.0,
            "absolute_height_in_km": (
                extent["min_absolute_height_in_km"]
                + extent["max_absolute_height_in_km"]
            )
            / 2.0,
        }
    ...
    def _get_geometry_xyz(
        self, latitude, longitude, absolute_height_in_km, correct_3dplane=False
    ):
        """
    ...
    def _get_timeshift_baz(
        ...
        """
        geom = self._get_geometry_xyz(
            latitude, longitude, absolute_height_in_km
        )
    ...
    def _get_timeshift(
        ...
        geom = self._get_geometry_xyz(latitude, longitude, absolute_height)

        geometry = self._geometry_dict_to_array(geom)

        if static3d:
            nstat = len(geometry)
            time_shift_tbl = np.empty(
                (nstat, grdpts_x, grdpts_y), dtype="float32"
            )
    ...
    def vespagram(
        ...
        starttime,
        endtime,
        reference="center_of_gravity",
        method="DLS",
        nthroot=1,
        static3d=False,
        vel_cor=4.0,
        wiggle_scale=1.0,
        density_cmap=obspy_sequential,
        plot="wiggle",
        show=True,
    ):
        """
            ...
            if plot == "wiggle":
                for i, (beam, slowness) in enumerate(zip(beams, slownesses)):
                    if i == beam_max:
                        ...
                        color = "k"
                        zorder = 1
                    ax.plot(t, slowness + scale * beam, color, zorder=zorder)
    ...
    def derive_rotation_from_array(
        self,
        stream,
        vp,
        vs,
        ...
        for value in list(components.values()):
            value.sort(
                key=lambda x: "%s.%s" % (x.stats.network, x.stats.station)
            )

        ids = [
            tuple([_i.id[:-1] for _i in traces])
            for traces in list(components.values())
        ]
        if len(set(ids)) != 1:
        ...
        if len(set(s)) != 1:
            raise ValueError(
                "starttime, npts, and sampling rate must be "
                "identical for all traces."
            )

        stations = [
            "%s.%s" % (_i.stats.network, _i.stats.station)
            for _i in list(components.values())[0]
        ]
        for station in stations:
            if station not in geo:
                raise ValueError(
                    "No coordinates known for station '%s'" % station
                )
        ...
        header = {
            ...
            "sampling_rate": list(components.values())[0][
                0
            ].stats.sampling_rate,
            "channel": "ROZ",
            "npts": len(d1),
    ...
    def slowness_whitened_power(
        ...
        """
        return self._array_analysis_helper(
            stream=stream,
            method="SWP",
            frqlow=frqlow,
            frqhigh=frqhigh,
    ...
    def _array_analysis_helper(
        ...
        if prefilter:
            st_workon.filter(
                "bandpass", freqmin=frqlow, freqmax=frqhigh, zerophase=True
            )
        else:
        ...
        filename_patterns = (
            os.path.join(tmpdir, "pow_map_%03d.npy"),
    ...
    def _covariance_array_processing(
        self,
        stream,
        win_len,
        win_frac,
        ...
        if len(stream) != len(stream.select(sampling_rate=fs)):
            msg = (
                "in array-processing sampling rates of traces in stream are"
                " not equal"
            )
            raise ValueError(msg)
        ...
        time_shift_table = self._get_timeshift(
            sll_x,
            sll_y,
            sl_s,
            grdpts_x,
        ...
        while eotr:
            try:
                for i, tr in enumerate(stream):
                    dat = tr.data[
                        spoint[i] + offset : spoint[i] + offset + nsamp
                    ]
            ...
            if relpow > semb_thres and 1.0 / slow > vel_thres:
                if timestamp == "julsec":
                    ...
                    outtime = UTCDateTime(
                        newstart.timestamp / (24.0 * 3600) + 719163
                    )
                else:
                    msg = (
                        "Option timestamp must be one of 'julsec',"
                        " or 'mlabday'"
                    )
                    raise ValueError(msg)
                res.append(np.array([outtime, relpow, abspow, baz, slow]))
                if verbose:
    ...
    def _three_c_dowhiten(fcoeffz, fcoeffn, fcoeffe, deltaf, whiten):
        ...
        for nst in range(fcoeffz.shape[0]):
            for nwin in range(fcoeffz.shape[1]):
                ...
                for k in range(3):
                    ampw[int(ww // 2) : int(n_freqs - ww // 2)] += (
                        csamp[ww:, k] - csamp[:-ww, k]
                    ) / ww
                # Fill zero elements at start and end of array with closest
                # non-zero value.
                ampw[n_freqs - int(ww // 2) :] = ampw[
                    n_freqs - int(ww // 2) - 1
                ]
                ...
                weight = np.where(
                    ampw > np.finfo(np.float).eps * 10.0,
                    1.0 / (ampw + np.finfo(np.float).eps),
                    0.0,
                )
    ...
    def _three_c_do_bf(
        ...
        theo_backazi = np.arange(0, 362, 2) * math.pi / 180.0
        ...
        ans = []
        for i, (tr_N, tr_E, tr_Z) in enumerate(
            zip(stream_n, stream_e, stream_z)
        ):
        ...
        if not out_wins > 0:
            msg = (
                "Zero output windows! Check data length, and parameters "
        ...
        for i in range(num_win):
            for n in range(n_stats):
                if (
                    not np.isnan(
                        _alldata_z[n, i * nstep : i * nstep + nsamp]
                    ).any()
                    and not np.isnan(
                        _alldata_n[n, i * nstep : i * nstep + nsamp]
                    ).any()
                    and not np.isnan(
                        _alldata_e[n, i * nstep : i * nstep + nsamp]
                    ).any()
                ):
                    # All data, tapered.
                    alldata_z[n, i, :] = _alldata_z[
                        n, i * nstep : i * nstep + nsamp
                    ] * cosine_taper(nsamp)
                    alldata_n[n, i, :] = _alldata_n[
                        n, i * nstep : i * nstep + nsamp
                    ] * cosine_taper(nsamp)
                    alldata_e[n, i, :] = _alldata_e[
                        n, i * nstep : i * nstep + nsamp
                    ] * cosine_taper(nsamp)
                    nst[i] += 1
        ...
        avg_starttime = UTCDateTime(
            np.mean([tr.stats.starttime.timestamp for tr in stream_n.traces])
        )
        window_start_times = np.array(
            [
                UTCDateTime(
                    avg_starttime + i * nstep / fs,
        ...
        index = np.where(
            (freq_range >= lowcorner) & (freq_range <= highcorner)
        )[0]
        fr = freq_range[index]
        ...
        fcoeffz = np.fft.fft(alldata_z, n=nsamp, axis=-1) / np.sqrt(
            fs * (cosine_taper(nsamp) ** 2).sum() * 0.5
        )
        fcoeffn = np.fft.fft(alldata_n, n=nsamp, axis=-1) / np.sqrt(
            fs * (cosine_taper(nsamp) ** 2).sum() * 0.5
        )
        fcoeffe = np.fft.fft(alldata_e, n=nsamp, axis=-1) / np.sqrt(
            fs * (cosine_taper(nsamp) ** 2).sum() * 0.5
        )
        fcoeffz = fcoeffz[:, :, index]
        fcoeffn = fcoeffn[:, :, index]
        fcoeffe = fcoeffe[:, :, index]
        deltaf = 1.0 / (nsamp * deltat)
        ...
        if whiten:
            ...
            if whiten >= fr[-1] - fr[0]:
                msg = (
                    "Moving frequency window is %s, it equals or exceeds "
        ...
        incs = np.arange(5, 90, 10) * math.pi / 180.0

        def pol_transverse(azi):
            pol_e = math.cos(theo_backazi[azi])
            pol_n = -1.0 * math.sin(theo_backazi[azi])
            return pol_e, pol_n
        ...
        beamres = np.zeros(
            (
                len(theo_backazi),
                u.size,
                max(out_wins, len(window_start_times)),
                nfreq,
            )
        )
            ...
            for win in range(0, out_wins * win_average, win_average):
                ...
                for vel in range(len(u)):
                    ...
                    e_steere = (
                        ...
                        * np.array(
                            [
                                ...
                                for azi in range(len(theo_backazi))
                            ]
                        )
                    ).T
                    e_steern = (
                    ...
                    if polarisation in [0, 1]:
                        ...
                        beamres[:, vel, int(win / win_average), f] = (
                            1.0
                            / (nst[win] * nst[win])
                            * abs((np.conjugate(w) * np.dot(r, wt).T).sum(1))
                        )
                        if coherency:
                            beamres[:, vel, int(win / win_average), f] /= abs(
                                np.sum(np.diag(r))
                            )
                            ...
                            w = np.concatenate(
                                (
                                    ...
                                    * np.cos(incs[inc_angle]),
                                    e_steern * np.sin(incs[inc_angle]),
                                    e_steere * np.sin(incs[inc_angle]),
                                ),
                                axis=1,
                            )
                            wt = w.T.copy()
                            ...
                            w = np.concatenate(
                                (
                                    ...
                                    * np.sin(incs[inc_angle]),
                                    e_steern * np.cos(incs[inc_angle]),
                                    e_steere * np.cos(incs[inc_angle]),
                                ),
                                axis=1,
                            )
                            wt = w.T.copy()
                ...
                if polarisation > 1:
                    i, j, k = np.unravel_index(
                        np.argmax(res[:, uindex, :]), res.shape
                    )
                    beamres[:, :, int(win / win_average), f] = res[:, :, k]
                    incidence[int(win / win_average), f] = (
                        incs[k] * 180.0 / math.pi
                    )

        return beamres, fr, incidence, window_start_times

    def three_component_beamforming(
        self,
        stream_n,
        stream_e,
        stream_z,
        ...
        if wavetype.lower() not in pol_dict:
            raise ValueError(
                "Invalid option for wavetype: {}".format(wavetype)
            )
        if (
            len(set(len(vel.traces) for vel in (stream_n, stream_e, stream_z)))
            > 1
        ):
            raise ValueError(
                "All three streams must have same number of " "traces."
            )
        ...
        starttime = max(
            ...
            for st in (stream_n, stream_e, stream_e)
        )
        min_starttime = min(
        ...
        if (
            max(abs(min_starttime - starttime), abs(max_endtime - endtime))
            > delta_common
        ):
            raise ValueError(
                "Traces do not have identical start/end times. "
        ...
        for st in (stream_n, stream_e, stream_z):
            for tr in st:
                if tr.stats.npts != npts_common:
                    raise ValueError(
                        "Traces do not have identical number of " "samples."
                    )
                if tr.stats.delta != delta_common:
                    raise ValueError(
                        "Traces do not have identical sampling " "rates."
                    )
        ...
        try:
            if wlen < smax * self.aperture:
                raise ValueError(
                    "Window length is smaller than maximum given"
            ...
            uindex = np.where(
                (u > 0.5 * smax + smin) & (u < 0.8 * smax + smin)
            )[0]
    ...
    def plot_radial_transfer_function(
        self, smin, smax, sstep, freqs, show=True
    ):
        """
        ...
        for f in freqs:
            ...
            for vel in range(len(u)):
                ...
                beamres[:, vel] = (
                    1.0
                    / (steering.shape[1] * steering.shape[1])
                    * abs((np.conjugate(w) * np.dot(r, wt).T).sum(1))
                )
            fig = plt.figure()
            ...
            ax.contour(theo_backazi, u, beamres.T, 40, cmap=cmap)
    ...
    def plot_transfer_function_freqslowness(
        self, slim, sstep, freq_min, freq_max, freq_step
    ):
        """
    ...
    def array_transfer_function_freqslowness(
        self, slim, sstep, fmin, fmax, fstep
    ):
        """
    ...
    def _array_transfer_function_helper(
        ...
        if isinstance(plim, float) or isinstance(plim, int):
            ...
            raise TypeError(
                "Parameter slim must either be a float "
                "or a tuple of length 4."
            )
        geometry = self._geometry_dict_to_array(
            self._get_geometry_xyz(**self.center_of_gravity)
        )
        npx = int(np.ceil((pxmax + pstep / 10.0 - pxmin) / pstep))
        npy = int(np.ceil((pymax + pstep / 10.0 - pymin) / pstep))
        transff = np.empty((npx, npy))

        if param == "wavenumber":
            for i, kx in enumerate(
                np.arange(pxmin, pxmax + pstep / 10.0, pstep)
            ):
                for j, ky in enumerate(
                    np.arange(pymin, pymax + pstep / 10.0, pstep)
                ):
                    _sum = 0j
                    for k in range(len(geometry)):
                        _sum += np.exp(
                            complex(
                                0.0, geometry[k, 0] * kx + geometry[k, 1] * ky
                            )
                        )
            ...
            for i, sx in enumerate(
                ...
                for j, sy in enumerate(
                    np.arange(pymin, pymax + pstep / 10.0, pstep)
                ):
                    for k, f in enumerate(
                        np.arange(fmin, fmax + fstep / 10.0, fstep)
                    ):
                        _sum = 0j
                        for l in np.arange(len(geometry)):
                            _sum += np.exp(
                                complex(
                                    0.0,
                                    (geometry[l, 0] * sx + geometry[l, 1] * sy)
                                    * 2
                                    * np.pi
                                    * f,
                                )
    ...
    def _beamforming(
        ...
        geometry = self._geometry_dict_to_array(
            self._get_geometry_xyz(
                correct_3dplane=correct_3dplane, **self.center_of_gravity
            )
        )
        ...
        spoint, _epoint = _get_stream_offsets(
            stream, (stime - mini), (etime - maxi)
        )
        ...
        while eotr:
                    ...
                    for y in range(grdpts_y):
                        ...
                        for i in range(nstat):
                            ...
                            try:
                                ...
                                singlet += (
                                    1.0 / nstat * np.sum(shifted * shifted)
                                )
                                beam += (
                                    1.0
                                    / nstat
                                    * np.power(np.abs(shifted), 1.0 / nthroot)
                                    * shifted
                                    / np.abs(shifted)
                                )
                            except IndexError:
                ...
                for x in range(grdpts_x):
                    for y in range(grdpts_y):
                        ...
                        for i in range(nstat):
                            ...
                            singlet += 1.0 / nstat * np.sum(shifted * shifted)
                            beam += (
                                1.0 / nstat * shifted * np.power(coh, nthroot)
                            )
                        bs = np.sum(beam * beam)
                        abspow_map[x, y] = bs / singlet
                        if abspow_map[x, y] > max_beam:
            ...
            if method == "SWP":
                ...
                for i in range(grdpts_x):
                    for j in range(grdpts_y):
                        abspow_map[i, j] = np.sum(
                            beam[:, i, j] / max_buffer[:], axis=0
                        ) / float(nf)

                beam_max = stream[0].data[
                    spoint[0] + offset : spoint[0] + nsamp + offset
                ]
            ...
            res.append(
                np.array(
                    [newstart.timestamp, abspow, baz, slow_x, slow_y, slow]
                )
            )
            if verbose:
                print(newstart, (newstart + (nsamp / fs)), res[-1][1:])
    ...
    def _vespagram_baz(
        ...
        mini = min(
            value.min()
            for key, value in time_shift_table.items()
            if key is not None
        )
        maxi = min(
            value.min()
        ...
        spoint, _ = _get_stream_offsets(
            stream, (starttime - mini), (endtime - maxi)
        )
        ...
        for _i, slowness in enumerate(slownesses):
            ...
            if method == "DLS":
                for _j, tr in enumerate(stream.traces):
                    ...
                    singlet += 1.0 / len(stream) * np.sum(shifted * shifted)
                    beams[_i] += (
                        1.0
                        / len(stream)
                        * np.power(np.abs(shifted), 1.0 / nthroot)
                ...
                beams[_i] = (
                    np.power(np.abs(beams[_i]), nthroot)
                    * beams[_i]
                    / np.abs(beams[_i])
                )
                ...
                for i in range(nstat):
                    ...
                    singlet += 1.0 / nstat * np.sum(shifted * shifted)
                    beams[_i] += 1.0 / nstat * shifted * np.power(coh, nthroot)
                bs = np.sum(beams[_i] * beams[_i])
                bs = bs / singlet
                if bs > max_beam:
                    max_beam = bs
    ...
    def _correct_with_3dplane(self, geometry):
        ...
        for _i, (key, value) in enumerate(sorted(list(orig_geometry.items()))):
            geodict[key] = {
                coord_sys_keys[0]: geometry[_i, 0],
